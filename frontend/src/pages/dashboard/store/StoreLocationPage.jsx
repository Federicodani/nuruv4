...
import { updateStoreLocation } from '../../../api/locationApi';
...
const StoreLocationPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateStoreLocation(coords.lat, coords.lng);
      setSaved(true);
    } catch (err) {
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-2xl font-bold text-secondary-900">Store Location</h1>
      <p className="mt-1 text-secondary-500">
        Set your store's physical location so clients can find you in nearby searches.
      </p>
      ...
      <div className="mt-8 flex flex-col gap-5">
        {status === 'idle' && (
          <div className="flex flex-col items-center gap-5 rounded-xl border border-dashed border-secondary-200 bg-secondary-50 py-12 text-center">
            ...
            <div>
              <h3 className="font-semibold text-secondary-900">Set Store Location</h3>
              <p className="mt-1 text-sm text-secondary-500">
                Visit your store and tap below to capture its GPS location.
              </p>
            </div>
            <Button icon={MapPin} onClick={requestLocation}>Detect Store Location</Button>
          </div>
        )}

        {status === 'requesting' && (
          <div className="rounded-xl border border-secondary-100 bg-white p-6 text-center shadow-card">
            <p className="text-secondary-600">Detecting location...</p>
          </div>
        )}
        ...
        {(status === 'denied' || status === 'error') && (
          <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-5">
            ...
            <Button variant="outline" size="sm" onClick={requestLocation} className="mt-3">Try Again</Button>
          </div>
        )}
          ...
          <div className="rounded-xl border border-secondary-100 bg-white p-5 shadow-card">
            ...
            {saved && (
              <div className="mt-3 flex items-center gap-2 rounded-lg bg-green-50 px-3 py-2 text-sm text-green-700">
                <CheckCircle className="h-4 w-4" /> Store location saved. You will now appear in nearby searches.
              </div>
            )}

            <div className="mt-4 flex gap-3">
              <Button onClick={requestLocation} variant="outline" size="sm" icon={Navigation}>Re-detect</Button>
              <Button onClick={handleSave} isLoading={saving} size="sm" icon={MapPin}>Save Location</Button>
            </div>
          </div>
        )}

        <div className="rounded-xl border border-secondary-100 bg-secondary-50 p-4 text-sm text-secondary-500">
          <strong className="text-secondary-700">Tip:</strong> For best accuracy, open this page while you are physically at your store.
          Clients will see your approximate distance from them, not your exact coordinates.
        </div>
      </div>
    </div>
  );
